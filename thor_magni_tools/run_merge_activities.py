...
from .preprocessing import ActivitiesMerger
...
parser = ArgumentParser(description="Trajectory Data and Activities Merger")

parser.add_argument(
    "--activities_path",
    type=str,
    required=True,
    help="Path to the activities raw file",
)
...
if run_batch:
    ...
    mergers = [
        ActivitiesMerger(
            activities_path=args.activities_path,
    ...
    merger = ActivitiesMerger(
        activities_path=args.activities_path, csv_path=files_path, out_dir=args.out_path
    )
    merger.run()
